...
BUILD_OUT_FOLDER = $(BUILD_FOLDER)/out
...
MAIN_BUILD_OUT_FOLDER = $(BUILD_OUT_FOLDER)/$(MAIN_PROJECT_NAME)
...
BT_BUILD_OUT_FOLDER = $(BUILD_OUT_FOLDER)/$(BT_PROJECT_NAME)
...
build:
	"$(ARDUINO_CLI)" compile $(VERBOSE) --build-path="$(MAIN_BUILD_FOLDER)" --fqbn $(MAIN_FQBN) --output-dir="$(MAIN_BUILD_OUT_FOLDER)" "$(MAIN_PROJECT_FOLDER)"
	"$(ARDUINO_CLI)" compile $(VERBOSE) --build-path="$(BT_BUILD_FOLDER)" --fqbn $(BT_FQBN) --output-dir="$(BT_BUILD_OUT_FOLDER)" "$(BT_PROJECT_FOLDER)"
